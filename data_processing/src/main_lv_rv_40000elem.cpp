...
// void readinp_tet_ex4_2(string Path, vector<vert> &nodes, vector<tet> &tets,
//                        vector<tri> &bou_epi_face, vector<tri> &bou_base_face,
...
void readinp_tet_4welems(vector<vert> &nodes, vector<tet> &tets,
                         vector<tri> &bou_epi_face, vector<tri> &bou_base_face,
                         vector<tri> &bou_endo_lv_face,
                         vector<tri> &bou_endo_rv_face) {
  // 读点和网格
  string Path = "./data/input/40000elem_lv_rv.inp";
  ifstream fin;
  fin.open(Path);
  if (!fin.good()) {
    cout << "file open failed";
...
int main() {
  cout << "begin" << endl;
  string file_name = "lv-rv-4welems";
  ...
  readinp_tet_4welems(verts, tets, bou_epi, bou_base, bou_endo_lv, bou_endo_rv);

  int nv = verts.size();  // num of vertexes

  // get fiber data
  string fiber_txt_path = "./data/input/40000fiber.txt";
  vert_fiber.resize(nv);
  vert_sheet.resize(nv);
  vert_normal.resize(nv);
  read_fiber_from_txt_ex4_2(fiber_txt_path, nv, vert_fiber, vert_sheet,
                            vert_normal);
