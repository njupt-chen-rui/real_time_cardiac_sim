...
if __name__ == "__main__":
    ...
    elec_model = elec.Electrophysiology_Aliec_Panfilov(body=geo_model)
    num_per_tet_set_np = np.array(meshData['sum_tet_set'], dtype=int)
    dyn_model = dyn.Dynamics_XPBD_SNH_Active(body=geo_model, num_pts_np=num_per_tet_set_np)

    mygui = gui.Gui(geometry_model=geo_model, electrophysiology_model=elec_model, dynamics_model=dyn_model,
                    body_name=body_name)
